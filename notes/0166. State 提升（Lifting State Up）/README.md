...
- [1. 本节内容](#1-本节内容)
- [2. 什么是 State 提升？](#2-什么是-state-提升)
  - [2.1. 基本概念](#21-基本概念)
  - [2.2. 数据流向](#22-数据流向)
- [3. 为什么需要提升 State？](#3-为什么需要提升-state)
  - [3.1. 原因 1：数据同步](#31-原因-1数据同步)
  - [3.2. 原因 2：单一数据源](#32-原因-2单一数据源)
  - [3.3. 原因 3：协调更新](#33-原因-3协调更新)
- [4. � 如何实现 State 提升？](#4--如何实现-state-提升)
  - [4.1. 步骤 1：识别共享状态](#41-步骤-1识别共享状态)
  - [4.2. 步骤 2：移动到共同父组件](#42-步骤-2移动到共同父组件)
  - [4.3. 步骤 3：通过 Props 传递](#43-步骤-3通过-props-传递)
  - [4.4. 完整示例](#44-完整示例)
- [5. State 提升的常见场景？](#5-state-提升的常见场景)
  - [5.1. 场景 1：表单输入同步](#51-场景-1表单输入同步)
  - [5.2. 场景 2：列表过滤和排序](#52-场景-2列表过滤和排序)
  - [5.3. 场景 3：多步骤表单](#53-场景-3多步骤表单)
  - [5.4. 场景 4：主从联动](#54-场景-4主从联动)
- [6. State 应该提升到哪里？](#6-state-应该提升到哪里)
  - [6.1. 原则：最近共同祖先](#61-原则最近共同祖先)
  - [6.2. 避免过度提升](#62-避免过度提升)
  - [6.3. 使用 Context 避免深层传递](#63-使用-context-避免深层传递)
  - [6.4. 状态位置决策流程](#64-状态位置决策流程)
- [7. 引用](#7-引用)
