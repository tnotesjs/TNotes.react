...
- [1. 本节内容](#1-本节内容)
- [2. react v18 都有哪些重大更新？（待整理）](#2-react-v18-都有哪些重大更新待整理)
  - [2.1. 🚀 核心主题：并发渲染（Concurrent Rendering）正式落地](#21--核心主题并发渲染concurrent-rendering正式落地)
  - [2.2. 新的 Root API：`createRoot`（强制升级点）](#22-新的-root-apicreateroot强制升级点)
  - [2.3. 自动批处理（Automatic Batching）](#23-自动批处理automatic-batching)
  - [2.4. 新 API：`startTransition`（标记非紧急更新）](#24-新-apistarttransition标记非紧急更新)
  - [2.5. `useDeferredValue` Hook](#25-usedeferredvalue-hook)
  - [2.6. Streaming SSR with Suspense（流式服务端渲染）](#26-streaming-ssr-with-suspense流式服务端渲染)
  - [2.7. `useId` Hook（服务端/客户端 ID 同步）](#27-useid-hook服务端客户端-id-同步)
  - [2.8. 严格模式（Strict Mode）增强](#28-严格模式strict-mode增强)
  - [2.9. Breaking Changes & 注意事项](#29-breaking-changes--注意事项)
  - [2.10. 其他改进](#210-其他改进)
  - [2.11. 总结](#211-总结)
- [3. 引用](#3-引用)
...
### 2.1. 🚀 核心主题：并发渲染（Concurrent Rendering）正式落地
...
### 2.2. 新的 Root API：`createRoot`（强制升级点）
...
### 2.4. 新 API：`startTransition`（标记非紧急更新）
...
### 2.6. Streaming SSR with Suspense（流式服务端渲染）
