...
- [1. 本节内容](#1-本节内容)
- [2. 版本控制的作用](#2-版本控制的作用)
- [3. Git 的基本工作流程](#3-git-的基本工作流程)
- [4. 初始化一个 Git 仓库](#4-初始化一个-git-仓库)
- [5. 暂存变更、提交变更](#5-暂存变更提交变更)
- [6. 其他常见的 Git 命令](#6-其他常见的-git-命令)
  - [6.1. 查看提交历史](#61-查看提交历史)
  - [6.2. 创建和切换分支](#62-创建和切换分支)
  - [6.3. 合并分支](#63-合并分支)
  - [6.4. 连接远程仓库](#64-连接远程仓库)
  - [6.5. 推送和拉取代码](#65-推送和拉取代码)
- [7. `.gitignore` 文件](#7-gitignore-文件)
- [8. 示例：React 项目的完整 Git 工作流](#8-示例react-项目的完整-git-工作流)
...
- 在实际开发中，分支策略（如 Git Flow）能够让多人协作更加顺畅，如果就是个人在维护的话，也可以简单粗暴一些，直接在默认的主分支 main/master 上提交。
- 养成频繁提交、编写清晰提交信息的习惯，能够帮助团队追踪问题和代码变更。
- 对于 React 项目，务必正确配置 `.gitignore`，避免将 `node_modules` 等目录提交到仓库，这个文件可以交给 Agent 去生成，提示词：“这是一个 React 学习仓库，请帮我初始化一份 `.gitignore` 文件”。
- 在初学 React 阶段，你可以先将重心都放在 React 直接相关的知识点上，关于 Git 只需要先有个初步了解，知道它主要用于解决什么问题，核心功能都有哪些即可，可以先不去刻意记 Git 的相关命令，直接通过提示词去引导 Agent 帮你完成相关 Git 操作。
...
# 2. 初始化 Git
git init # vite 已自动初始化为 git 仓库的话，则无需执行
# 判断：可以通过 git status 查看 git 创库状态，如果这并非一个 git 仓库，则会报错
