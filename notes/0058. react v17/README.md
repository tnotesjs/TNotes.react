...
- [1. 本节内容](#1-本节内容)
- [2. react v17 都有哪些重大更新？（待整理）](#2-react-v17-都有哪些重大更新待整理)
  - [2.1. 核心目标： 为多版本共存和渐进升级做准备](#21-核心目标-为多版本共存和渐进升级做准备)
  - [2.2. 新的 JSX 转换（New JSX Transform） （可选但推荐）](#22-新的-jsx-转换new-jsx-transform-可选但推荐)
  - [2.3. 事件委托从 `document` 改为 React 树的根容器](#23-事件委托从-document-改为-react-树的根容器)
    - [旧行为（React 16 及之前）](#旧行为react-16-及之前)
    - [新行为（React 17+）](#新行为react-17)
    - [潜在影响](#潜在影响)
  - [2.4. 更一致的 `useEffect` 清理函数执行时机](#24-更一致的-useeffect-清理函数执行时机)
  - [2.5. 错误处理更严格](#25-错误处理更严格)
  - [2.6. 移除一些过时的生命周期警告](#26-移除一些过时的生命周期警告)
  - [2.7. Breaking Changes（破坏性变更）](#27-breaking-changes破坏性变更)
  - [2.8. 其他改进](#28-其他改进)
  - [2.9. 总结](#29-总结)
- [3. 引用](#3-引用)
...
### 2.3. 事件委托从 `document` 改为 React 树的根容器
